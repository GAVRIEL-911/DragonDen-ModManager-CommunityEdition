...
public static class InstalledScanner
{
    public static ImportStats ImportFromDisk()
    {
        ...
        // SPT 4.1+ uses SPT_Runtime/user/mods; older builds used SPT/user/mods or user/mods
        var serverModsRuntime = Path.Combine(sptRoot, "SPT_Runtime", "user", "mods");
        ...
        if (Directory.Exists(serverModsRuntime))
            ScanRoot(sptRoot, serverModsRuntime, "SPT_Runtime/user/mods", Installer.Target.Server, ref imported, ref updated, ref skipped);
    ...
    private static void ScanRoot(string sptRoot, string scanRoot, string scanPrefix, Installer.Target target, ref int imported, ref int updated, ref int skipped)
    {
        ...
        foreach (var dir in Directory.EnumerateDirectories(scanRoot))
        {
            ...
            string version;
            if (!string.IsNullOrWhiteSpace(versionFromDb) && !string.Equals(versionFromDb, "Custom Install", StringComparison.OrdinalIgnoreCase))
            {
                version = versionFromDb;
            }
            else
            {
                // Prefer BepInPlugin attribute version, then FileVersion, then folder detection
                ...
                var chosenDll = dllsInFolder.FirstOrDefault(d =>
                    ...
                    ?? dllsInFolder.FirstOrDefault();
                if (!string.IsNullOrWhiteSpace(chosenDll))
                {
                    var bep = DllIntrospector.TryGetBepInExInfo(chosenDll);
                    if (bep != null && !string.IsNullOrWhiteSpace(bep.Version))
                        detected = SemverUtil.NormalizeToThreeParts(bep.Version);
                    if (string.IsNullOrWhiteSpace(detected))
                        detected = DllVersion.FromDll(chosenDll);
                }
                if (string.IsNullOrWhiteSpace(detected))
                    detected = DllVersion.DetectFromFolder(dir);
                version = !string.IsNullOrWhiteSpace(detected) ? detected : "Custom Install";
            }
        ...
        if (string.Equals(scanPrefix, "BepInEx/plugins", StringComparison.OrdinalIgnoreCase))
        {
            ...
            foreach (var dll in looseDlls)
            {
                ...
                string version;
                if (App.Db.TryGetInstalled(stem, out var v) && !string.IsNullOrWhiteSpace(v) && !string.Equals(v, "Custom Install", StringComparison.OrdinalIgnoreCase))
                {
                    version = v!;
                }
                else
                {
                    string? detected = null;
                    var bep = DllIntrospector.TryGetBepInExInfo(dll);
                    if (bep != null && !string.IsNullOrWhiteSpace(bep.Version))
                        detected = SemverUtil.NormalizeToThreeParts(bep.Version);
                    if (string.IsNullOrWhiteSpace(detected))
                        detected = DllVersion.FromDll(dll);
                    version = !string.IsNullOrWhiteSpace(detected) ? detected : "Custom Install";
                }
