...
public static class Installer
{
        ...
        IProgress<(string phase, int pct)>? progress = null, InstallContext? ctx = null, CancellationToken ct = default)
    {
        ...
        {
            Notifications.Current.ShowError("安装失败", "未配置 SPT 根目录或不存在");
        ...
        {
            Notifications.Current.ShowError("安装失败",
        ...
        static bool IsServerPath(string rel)
        {
            ...
            return u.StartsWith("spt/user/mods/", StringComparison.OrdinalIgnoreCase)
                   ...
                   || u.StartsWith("spt_runtime/user/mods/", StringComparison.OrdinalIgnoreCase)
    ...
    private static Intent DetectIntent(List<string> entries)
    {
        ...
        foreach (var e in entries)
        {
            ...
            if (p.StartsWith("SPT/user/mods/", StringComparison.OrdinalIgnoreCase) ||
                p.StartsWith("SPT_Runtime/user/mods/", StringComparison.OrdinalIgnoreCase) ||
            ...
            if (string.Equals(top, "SPT", StringComparison.OrdinalIgnoreCase)
                || string.Equals(top, "SPT_Runtime", StringComparison.OrdinalIgnoreCase))
                intent.serverLikely = true;
        }

        if (intent.clientLikely && intent.clientRoots.Count == 0) intent.clientRoots.Add("BepInEx/plugins");
        if (intent.serverLikely && intent.serverRoots.Count == 0)
            intent.serverRoots.Add(Spt.DetectServerModsRelative());
        return intent;
    }
