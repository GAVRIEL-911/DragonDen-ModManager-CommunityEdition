...
public partial class InstalledModsPage : UserControl
{
    ...
    private async Task UpdateAll()
    {
        ...
        foreach (var g in groups)
        {
            ...
            try
            {
                ...
                {
                    var lv = latestForAB.Version ?? "自定义安装";
    ...
    {
        if (!_isBuilding) SetStatus("正在扫描模组…");
    ...
    private void OnUpdateFromBadge(object? sender, RoutedEventArgs e)
    {
        ...
        {
            b.IsEnabled = false;
            App.Queue.EnqueueRemote(row.Name, url, row.Latest?.Version ?? "自定义安装", row.Guid ?? "");
    ...
    private static string BuildFikaLabel(string installedVersion, List<ForgeClient.ModVersion> allVersions, ForgeClient.ModVersion? abLatest)
    {
        ...
        if (kindInstalled == "compatible") return "Fika 兼容（已安装）";
        if (kindInstalled == "incompatible") return "Fika 不兼容（已安装）";

        if (abLatest != null)
        {
            var kindLatest = NormalizeFika(abLatest.FikaCompatibility);
            if (kindLatest == "compatible") return "Fika 兼容（最新版）";
            if (kindLatest == "incompatible") return "Fika 不兼容（最新版）";
        }

        var anyCompat = allVersions.Any(v => NormalizeFika(v.FikaCompatibility) == "compatible");
        if (anyCompat) return "Fika 兼容（其他版本）";

        if (allVersions.Count > 0) return "Fika 状态未知";
