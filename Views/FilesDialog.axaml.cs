...
public partial class FilesDialog : Window
{
    ...
    private void OnFilesListDoubleTapped(object? s, TappedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                Notifications.Current.ShowWarning("打开失败", "文件路径无效。");
                return;
            }

            if (!File.Exists(path) && !Directory.Exists(path))
            {
                Notifications.Current.ShowWarning("打开失败", "磁盘上找不到该文件。");
            ...
            catch (Exception first)
            {
                ...
                {
                    Notifications.Current.ShowError("打开失败", $"Couldn't open the file '{Path.GetFileName(path)}'.");
        ...
        {
            Notifications.Current.ShowError("打开失败", "打开文件时发生意外错误。");
