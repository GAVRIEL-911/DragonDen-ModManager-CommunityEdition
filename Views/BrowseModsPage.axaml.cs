...
public partial class BrowseModsPage : UserControl
{
    ...
    private static bool TryFindSptExeForRoot(string root, out string exePath)
        => Spt.TryFindAnyServerExe(root, out exePath);
    ...
    private async Task LoadCategoriesThenSearch()
    {
        ...
        {
            Notifications.Current.ShowError("分类加载失败", "无法加载分类。");
    ...
    private async Task<List<UiCategory>> FetchCategoriesFromCache()
    {
        ...
        var items = new List<UiCategory> { new() { Title = "全部分类", Slug = "", ColorClass = "" } };
    ...
    private void SetCategories(List<UiCategory> items)
    {
        ...
        var match = items.FirstOrDefault(i => string.Equals(i.Slug, prevSlug, StringComparison.OrdinalIgnoreCase))
                    ...
                    ?? new UiCategory { Title = "全部分类", Slug = "" };
    ...
    private async Task StartIndexingAsync(bool showModal)
    {
        ...
        catch (Exception ex)
        {
            ...
            _isIndexing = false;
            UpdatePagingUi();
            Notifications.Current.ShowError("刷新失败", "无法刷新模组缓存或 Forge 数据。");
    ...
    private async Task PerformSearch(bool resetPage, bool softOverlay = false)
    {
                ...
                if (m.source_code_links is { Length: > 0 })
                {
                    foreach (var s in m.source_code_links)
                    {
                        if (s is { url: { } u } && !string.IsNullOrWhiteSpace(u))
                        {
                            ...
                            {
                                Url = u,
                                Label = string.IsNullOrWhiteSpace(s.label) ? "来源" : s.label!.Trim()
                            });
                        }
                    }
                }
        ...
        {
            Notifications.Current.ShowError("搜索失败", "无法完成搜索。");
    ...
    private async Task<List<ForgeClient.MissingDep>> ResolveMissingDependenciesAsync(int modId, int versionId)
    {
        ...
        {
            Notifications.Current.ShowError("依赖获取失败",
        ...
        {
            Notifications.Current.ShowError("依赖获取失败", "无法获取模组依赖。");
            Logger.Error("[BrowseModsPage] Dependency fetch failed: " + ex.Message);
        }
        ...
        List<ForgeClient.MissingDep> missing)
    {
        ...
        foreach (var dep in required)
        {
            ...
            try
            {
                ...
                App.Queue.EnqueueRemote(string.IsNullOrWhiteSpace(dep.Name) ? dep.Guid ?? "Dependency" : dep.Name,
                    best.Link!, best.Version ?? "自定义安装", dep.Guid ?? "");
        ...
        App.Queue.EnqueueRemote(modName, selectedVersion.Link!, selectedVersion.Version ?? "自定义安装", "");
    ...
    private void OnOpenSource(object? sender, RoutedEventArgs e)
    {
        ...
        {
            Notifications.Current.ShowError("打开失败", "无法打开模组源码链接。");
    ...
    private void OnOpenModPage(object? sender, RoutedEventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(url))
        {
            ...
            {
                Notifications.Current.ShowError("打开失败", "无法在浏览器中打开模组页面。");
                Logger.Error("[BrowseModsPage] Couldn't open mod page: " + ex.Message);
            }
        }
        else
        {
            Notifications.Current.ShowWarning("缺少链接", "未找到该模组的 Forge 页面链接。");
    ...
    private async void OnCopyLink(object? sender, RoutedEventArgs e)
    {
        ...
        if (string.IsNullOrWhiteSpace(url))
        {
            Notifications.Current.ShowWarning("缺少链接", "没有可复制的链接。");
            Logger.Warn("[BrowseModsPage] 没有可复制的链接。");
            return;
        }
        ...
        try
        {
            ...
            if (tl?.Clipboard is not null)
            {
                ...
                Notifications.Current.ShowSuccess("已复制", "模组链接已复制到剪贴板。");
            }
        }
        catch (Exception ex)
        {
            Notifications.Current.ShowError("复制失败", "无法复制模组链接。");
    ...
    private async void OnCopyName(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (tl?.Clipboard != null)
            {
                ...
                Notifications.Current.ShowSuccess("已复制", "模组名称已复制到剪贴板。");
                Logger.Info($"[BrowseModsPage] 已复制 mod name: {name}");
            }
        }
        catch (Exception ex)
        {
            Notifications.Current.ShowError("复制失败", "无法复制模组名称。");
    ...
    private async void OnCopyGuid(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (tl?.Clipboard != null)
            {
                ...
                Notifications.Current.ShowSuccess("已复制", "模组 GUID 已复制到剪贴板。");
                Logger.Info($"[BrowseModsPage] 已复制 mod GUID: {guid}");
            }
        }
        catch (Exception ex)
        {
            Notifications.Current.ShowError("复制失败", "无法复制 GUID。");
    ...
    private void OnOpenThumb(object? sender, RoutedEventArgs e)
    {
        ...
        {
            Notifications.Current.ShowError("打开失败", "无法打开缩略图。");
    ...
    private async void OnInstallSelected(object? sender, RoutedEventArgs e)
    {
        ...
        if (row is null)
        {
            Notifications.Current.ShowWarning("未选择模组", "无法确定要安装的模组。");
            return;
        }
        ...
        {
            Notifications.Current.ShowWarning("缺少链接", "该版本没有可用的下载链接。");
            return;
        }
        ...
        if (missing.Count > 0 && !row.IsInstalled)
        {
            ...
            {
                App.Queue.EnqueueRemote(row.Name, chosen.Link!, chosen.Version ?? "自定义安装", row.Guid ?? "");
                MarkRowsQueued(new[] { (row.ModId, row.Name, row.Guid) });
            }
        }
        else
    ...
    private void ApplyLanguage()
    {
        try
        {
            ...
            if (PrevBtn != null) PrevBtn.Content = Loc.T("Common.Prev");
            if (NextBtn != null) NextBtn.Content = Loc.T("Common.Next");
            if (HideInstalledChk != null) HideInstalledChk.Content = Loc.T("Browse.HideInstalled");
            if (FeaturedChk != null) FeaturedChk.Content = Loc.T("Browse.HideFeatured");
            if (AdsChk != null) AdsChk.Content = Loc.T("Browse.HideAds");
            if (AiChk != null) AiChk.Content = Loc.T("Browse.HideAI");
            if (FikaOnlyChk != null) FikaOnlyChk.Content = Loc.T("Browse.FikaOnly");
            // Force template re-eval for LocKey bindings
            if (ResultsList != null)
            {
        ...
        catch (System.Exception ex) { Logger.Error($"[Browse] ApplyLanguage: {ex.Message}"); }
    }
