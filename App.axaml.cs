...
public class App : Application
{
    ...
    public override void OnFrameworkInitializationCompleted()
    {
        ...
        // Force-migrate legacy Forge domain if still present in user config
        try
        {
            var bu = Config.Forge?.BaseUrl ?? "";
            if (bu.Contains("forge.sp-tarkov.com", StringComparison.OrdinalIgnoreCase)
                || (bu.Contains("sp-tarkov.com", StringComparison.OrdinalIgnoreCase)
                    && !bu.Contains("sp-mod.com", StringComparison.OrdinalIgnoreCase))
                || string.IsNullOrWhiteSpace(bu))
            {
                Config.Forge.BaseUrl = "https://sp-mod.com";
                Config.Save(Paths.AppSettingsPath);
                Logger.Info("[App] Migrated Forge BaseUrl → https://sp-mod.com");
            }
            // Prefer SPT_Runtime relative path when still on legacy SPT/
            var sm = Config.Paths?.ServerModsRelative ?? "";
            if (string.IsNullOrWhiteSpace(sm) || sm.Equals("SPT/user/mods", StringComparison.OrdinalIgnoreCase))
            {
                Config.Paths.ServerModsRelative = "SPT_Runtime/user/mods";
                Config.Save(Paths.AppSettingsPath);
            }
        }
        catch (Exception ex)
        {
            Logger.Error($"[App] Config migration failed: {ex.Message}");
        }

        // Initialize localization (en / zh-CN / zh-TW)
        try
        {
            DragonDen.ModManager.Services.Localization.Loc.InitializeFromConfig();
        }
        catch (Exception ex)
        {
            Logger.Error($"[App] Loc init failed: {ex.Message}");
        }
